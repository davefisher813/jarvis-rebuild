...
// printed was measured against whatever happened to be on port 4177, and a
...
//   DEMO=1 npm run build && npx vite preview --port 4177
const DEMO_BUILD = await fetch("http://localhost:4177/DEMO_BUILD")
  .then((r) => r.ok).catch(() => false);
const SEED_LINE = DEMO_BUILD
  ? "SEEDED BUILD: demo data is present, so this measures full screens."
  : "EMPTY BUILD: no DEMO_BUILD marker on :4177, so this measured EMPTY STATES.\n" +
    "  Rebuild with: DEMO=1 npm run build && npx vite preview --port 4177";
...
async function runPass({ w, h, theme, scale = 1 }) {
  ...
  try {
    ...
    const resetApp = async () => {
      await page.goto("http://localhost:4177/", { waitUntil: "networkidle" }).catch(() => {});
...
writeFileSync("/tmp/claude-0/-home-user-jarvis-rebuild/c2bd2ca3-55ff-5b99-b510-cb004d9cbd80/scratchpad/notesbrain/audit.json", JSON.stringify(passes, null, 1));
