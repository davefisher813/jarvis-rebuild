...
export const CHECKS: Check[] = [
  ...
  { group: "Google", name: "calendar import is idempotent (dedupe by id)", run: async () => {
      ...
      eq((await importCalendar(api, sched)).created, 1, "first import"); eq((await importCalendar(api, sched)).created, 0, "re-import dedupes"); } },
  ...
  { group: "Admin", name: "real api is unavailable until something proves the server answered", run: () => { ok(createAdminApi("t").available === false, "not configured by default"); ok(createAdminApi("t", true).available === true, "available once the caller's probe answered"); } },
