...
import { bucketOf, closable, projStatus, rankGoals } from "./progress";
...
// the Brain tab lists -- with goals homed by their first tag and projects by
// their category, each project saying the goal it is filed to on its own
// row. One tree, two lenses: Brain is the stuff in an area, this page is the
// direction of it.
//
...
export default function BiggerPicturePage({
  ...
  // PICK 20: what a row could never say, derived by the flow: a hold's date.
  ...
  // Each lens is one kind of thing and is empty on its own terms. The single
  // frame that held both was never mounted once Life passed its segments, and
  // went on 2026-09-26 (§AK): its rows stacked up to five greys.
  const empty = projectsLens ? projectRows.length === 0 : goals.length === 0;
  if (empty) {
  ...
  // On the Projects lens nothing nests: every open project lands under its
  // category with its goal on its own line, and one whose category is not a
  // live area lands in More Work.
  const orphanRows = openRows.filter((r) => !sectionIds.has(r.project.data.category ?? ""));
