...
export default function AppShell({ seedDemo = false }: { seedDemo?: boolean }) {
  ...
  // START NOW FROM TODAY (Dave 2026-09-17): the Start screen lives on the
  // Tasks tab, so Today hands it the task and goes there.
  const startIntent = useOneShot<string>();
  ...
  const [lifeSegment, setLifeSegment] = useState<LifeSegment | undefined>(undefined);
  ...
  const goLife = (seg: LifeSegment) => { taskFilterIntent.clear(); setLifeSegment(seg); setLifeNav((n) => n + 1); setActive("life"); };
