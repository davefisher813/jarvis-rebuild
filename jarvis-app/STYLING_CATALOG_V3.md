...
- **Late is red from the first late thing.** The key has no threshold, so the Today late tile is red at any count and the due tile is amber (needs you soon). The tile still does not exist until something has slipped. Law L1 (`laws.test.ts`) now forbids only the BRAND red (`--accent*`, `--tint`, `--on-light-red`) on a lateness class, because lateness in `--sys-red` is the key (late, overdue and missed; missed joined the check on the round-2 review); an unread or behind class takes no red at all, the system red included, because behind is stalled work and the key files that under amber. `browserWalk.test.ts` pins light `--sys-red` to Apple's `#FF3B30` and fails a light rule that steps lateness down to the words red.
