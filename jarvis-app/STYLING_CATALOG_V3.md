...
10. BLACK STEEL CANVAS, NUMBERED (Dave's final pick, 2026-08-19: "5 with 4's numbering"): every document heading renders as a red counter number (12px bold tabular, 65% opacity, CSS counter so it renumbers itself) followed by the heading in red mini-caps (12px, bold, 0.14em tracking, uppercase via CSS) and a dotted red leader running to the margin. Doc titles stay editorial: 30px at -0.02em; body 17px at 1.65. Tables are steel: surfaced card, quiet header band, red-washed Total row with the accent total. This is the writing canvas's one styling; a section head without its number, red caps, and leader is a violation.
